...
function ShareBioAge() {
    ...
    if (bioAgeComponent) {
      try {
        ...
        if (navigator.canShare && navigator.canShare(shareData)) {
          ...
          alert('Chart only downloadable for device. Please check downloads folder');
        }
      } catch (error) {
  ...
  const handleDownloadChart = async () => {
    ...
    if (bioAgeComponent) {
      try {
        ...
        downloadLink.click();
        document.body.removeChild(downloadLink);
        alert('Chart image downloaded successfully.');
      } catch (error) {
        console.error('Error capturing BioAge component:', error);
        setShareError('Error capturing the chart image.');
      }
  ...
  return (
      <div>
        ...
        <button onClick={handleDownloadChart} disabled={sharing}>
          Download
        </button>
        {shareError && <div className="error">{shareError}</div>}
      </div>
